...
const RoomPage = ({ onCreateRoom }) => {
  ...
  const handleToggleFavorite = (id) => {
    setSelectedRestaurants((prevState) => {
      const index = prevState.findIndex((r) => r.id === id);
      if (index === -1) {
        return [...prevState, restaurants.find((r) => r.id === id)];
      } else {
        const newSelectedRestaurants = [...prevState];
        newSelectedRestaurants.splice(index, 1);
        return newSelectedRestaurants;
      }
    });
  };
